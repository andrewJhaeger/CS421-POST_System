...
/**
 * This class serves as a controller between the GUI and the processing side of
 * the POST system.
 */
public class Register {
  ...
  /**
   * Defines format strings for receipt output and accepts a given product
   * catalog.
   *
   * @param inCatalog
   */
   ...
   * The receipt is an ArrayList of Strings and is updated in real time so it
   * can be displayed to the user while being processed.
   */
  public void makeNewSale(int registerNum) {
    ...
    receipt.add(String.format("%-28s %s\n", "Receipt No. " 
            + sale.getReceiptNumber(), dateFormat.format(getDate().getTime())));
  ...
  /**
   * Initializes a Sale object to be used as a ledger for the total items and
   * cost of items sold over a certain day.
   *
   * @param date
   */
   ...
   * specification of the desired item, then adds a line item to the sale. Also
   * checks for already existing items in the sale, and adds to their quantity
   * instead of adding a duplicate line item.
   *
   * @param itemID
   * @param quantity
   */
  ...
  /**
   * Removes an item from the sale by itemID. Also removes its line from the
   * receipt.
   *
   * @param itemID
   */
  ...
  public void endSale() {
    receipt.add("---------------------------------------------");
    receipt.add(String.format(receiptBottomFormat, "Subtotal: " 
            + sale.getTotal().stringValue()));
    Money money = new Money(sale.getTotal());
    money.multiply(0.06);
    receipt.add(String.format(receiptBottomFormat, "Tax (6%): " 
            + money.stringValue()));
    money.add(sale.getTotal());
    receipt.add(String.format(receiptBottomFormat, "Total: " 
            + money.stringValue()));
  }

  /**
   * Creates a payment from the tendered amount and completes the sale. Then
   * adds final lines to receipt and writes the receipt to an external file.
   *
   * @param tenderedAmt
   */
  ...
  /**
   * Writes the receipt to a text file in a directory according to the date the
   * file was created. The file is named by its receipt number.
   */
  private void writeReceiptToFile() {
    SimpleDateFormat folderDateFormat = new SimpleDateFormat("MM-dd-yy");
    File dir = new File("receipts/" 
            + folderDateFormat.format(getDate().getTime()));
    if (!dir.exists()) {
      dir.mkdirs();
    }
  ...
  /**
   * Reads in the data from a receipt to create a new sale item to add all the
   * items into one sale, completing he daily report
   *
   * @param receipt
   * @throws FileNotFoundException
   * @throws IOException
   */
  public void readItemsFromReceipt(File receiptFile) 
          throws FileNotFoundException, IOException {
    Scanner fileScanner = new Scanner(new BufferedReader(
            new FileReader(receiptFile))).useDelimiter("\\s{2,}");
    int itemNum;
    int quantity;
  ...
  /**
   * Reads in the data from a receipt to print out the items in a list so the
   * user can return an item
   *
   * @param receiptNumber
   * @return Money[]
   */
      ...
      for (File subDir : dir.listFiles()) {
        if (subDir.isDirectory()) {
          for (File file : subDir.listFiles()) {
            //If the file matches the receipt number
            if (file.getName().equals(receiptNumber + ".txt")) {  
              BufferedReader br = new BufferedReader(new FileReader(file));
              while ((line = br.readLine()) != null) {
                if (line.startsWith("1")) {  //If we are at the first item line
                  //While we are not at the end of the sale items
                  while (!line.startsWith("-")) {  
                    lineVars = line.split("\\s{2,}");
                    enterItem(Integer.parseInt(lineVars[0]), 
                            Integer.parseInt(lineVars[2]));
                    line = br.readLine();
                  }
                } else {
                  ...
                  if (line.contains("Tendered Amount:")) {
                    String oldTenderedString = line.replace("Tendered Amount: $"
                            , "").trim();
